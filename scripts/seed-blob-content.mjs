 ...
 *
 * Needed because switching the CMS over to Blob storage (BLOB_READ_WRITE_TOKEN set) means
 * production reads go through Blob only — it doesn't know about content that was only ever
 * written to the repo filesystem in local dev.
 *
 * Usage: pull the token from Vercel first (`vercel env pull .env.local`), then:
